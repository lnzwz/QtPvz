...
public:
    ...
    void SolveNd();
    void MakeBo(int,int);
    void MakeZMBP(int gq,int gq_nd,int lx);
    int MakeGq(int gq);
    ...
    void SendTick();
    void GetNxtDbTm();
    void CheckMFZ(int xz[MAXXUAN],int n);
    void ResetIcons();
    int ChooseGQ();
    void EndLessNd();
...
public:
    QTimer icotmr,drtmr,uptmr;//图标刷新，绘图（多线程），界面刷新
    ...
    int select,sele_x,sele_y;//选中的植物，选中的位置
    ...
    bool can_wat[MAX_ID_CNT];//（僵尸）是否能进水
    double cd_spd[MAXSHU];//恢复速度
    int m_jwbs[MMN+1];//僵王血量
    int ZWSHU,JSH,ND;//植物种类数，僵尸种类总数，波数
    bool is_boss,is12,is_coll;//是否为两种BOSS关卡和82
    ...
    int chui_sc,f_shu,z_fs,w_tm,h_sd,v_tm;//狂锤分数，待生成的化肥数目，剩余化肥数目，，回放速度，加速模式虚拟时间
    ...
    bool del_js[MAXJSH],auto_fire,crea,b_two;bool hiui=0;
    MyGLWidget gl;int drty,js_od[22][12],ercn;bool xie,noztm;QCursor cur;
    int laSI,jsty[MAXJSH],lamul[MAXJSH],dbn,zdbs,sun_tm,usermn,usermm,usersi,userlx,bgid;
    bool jshd[MAXJSH],b_kjc,b_sun,low_eff,b_snd,swizw,sp_plt[MAXSHU],inf_sun;
    double dps_sum[10000],dps_now;bool b_sho,ena_fei;
    bool not_imjsz[MAXSHU];
    QSignalMapper tcpmap,rdmap;

    double js_cz[MAXJSH];int js_nw[1000],js_nwi;

    int dbg=0,dbg_tm=1e9;QString dbg_str;
